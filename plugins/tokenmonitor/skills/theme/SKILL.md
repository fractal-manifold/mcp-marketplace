...
`tokenmonitor_list_devices` and ask which one to retarget (skip the
...
> `tokenmonitor_list_devices` afterwards to confirm.

If the user wants to verify immediately, suggest
`tokenmonitor_recent_logs` to watch for the `rebooting to apply
...
1. `tokenmonitor_list_devices`     — resolve the target device.
2. `tokenmonitor_set_device_pending` — queue `theme_mode`.
...
- **`device <id> not registered`** — the device was never registered
  on this broker. Run `/tokenmonitor:configure` first, or
  `tokenmonitor_register_device` if it was registered elsewhere.
- **`registry disabled`** — the user's broker is running without a
  registry path. They need to configure
  `~/.config/tokenmonitor/devices/` and restart the broker.
