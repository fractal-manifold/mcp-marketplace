// tmon-setconfig — DEV-ONLY helper to stage a CONFIG pending (theme / autorotate)
// directly via the registry package, the same way tokenmonitor_set_device_pending
...
func main() {
	...
	if home, err := os.UserHomeDir(); err == nil {
		def = filepath.Join(home, ".config", "tokenmonitor", "devices")
	}
	...
	if *id == "" {
		fmt.Fprintln(os.Stderr, "usage: tmon-setconfig -id <id> [-theme day|night|auto] [-rotate-s 1..300] [-rotate-enabled true|false] [-dir <devices>]")
		os.Exit(2)
	}
