...
[TokenMonitor](https://github.com/fractal-manifold/tokenmonitor)
...
[releases page](https://github.com/fractal-manifold/tokenmonitor-mcp/releases)
once they're cut. Confirm with:

```sh
tokenmonitor-mcp --version
```
...
**Legacy compatibility**: if `tokenmonitor.toml` is missing, `tokenmonitor-mcp` falls back to
`~/.config/tokenmonitor/service.toml` (same schema), so existing
...
```sh
claude mcp add tokenmonitor-mcp -- "$(command -v tokenmonitor-mcp)"
```
...
```json
{
  "mcpServers": {
    "tokenmonitor-mcp": {
      "command": "/absolute/path/to/tokenmonitor-mcp"
    }
  }
}
```
...
Writes `[mcp_servers.tokenmonitor-mcp]` to `~/.codex/config.toml`. Verify with
`codex mcp list` and `codex mcp get tokenmonitor-mcp` — transport should read
...
```ini
[Unit]
Description=TokenMonitor credential broker
...
Per-device state lives under `~/.config/tokenmonitor/devices/`,
...
AND reports `X-Tmon-Config-Version == pending.version`. Both conditions
...
`_tmon._tcp.local.` (TXT record `device_id`, `state=needs_config`, `fw`)
...
curl -sS http://127.0.0.1:8765/credentials \
  -H "X-Tmon-Timestamp: ${TS}" \
  -H "X-Tmon-Nonce: ${NONCE}" \
  -H "X-Tmon-Signature: ${SIG}"
```
