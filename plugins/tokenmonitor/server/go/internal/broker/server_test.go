...
func newTestConfigWithCodex(t *testing.T, credsPath string, codexEnabled bool, codexPath string) *config.Config {
	...
	dir := t.TempDir()
	tomlPath := filepath.Join(dir, "tokenmonitor.toml")
	body := `
[server]
bind = "127.0.0.1"
port = 0
...
func newCodexTestServer(t *testing.T, codexPath string, enabled bool) (*httptest.Server, *config.Config) {
	...
	ts := httptest.NewServer(NewMux(cfg, cache, state.New(), logger, nil, nil, nil, nil, nil))
	t.Cleanup(ts.Close)
	return ts, cfg
}
...
func signedRequestPath(t *testing.T, ts *httptest.Server, cfg *config.Config, path string, nonce string) *http.Response {
	...
	req.Header.Set("X-Tmon-Timestamp", tsHdr)
...
func TestServer_WrongSignature(t *testing.T) {
	...
	req.Header.Set("X-Tmon-Timestamp", strconv.FormatInt(now, 10))
	req.Header.Set("X-Tmon-Nonce", "0123456789abcdef0123456789abcdef")
	req.Header.Set("X-Tmon-Signature", "deadbeef")
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	}
