...
// agree on the same lock path and cannot both grab the port.
func lockDir() string {
	euid := os.Geteuid()
...
func openRawSerial(path string) (*os.File, error) {
	...
	// against a CAP_SYS_ADMIN opener, which is why the flock above is the real
	...
	// before we get here, so this only narrows (not eliminates) the window —
	// firmware tolerates an unexpected reset as the real mitigation.
...
// equivalent). Baud is left untouched because USB-CDC ignores it.
func setRawSerial(fd int) error {
	tio, err := unix.IoctlGetTermios(fd, unix.TCGETS)
	if err != nil {
		return fmt.Errorf("usbprov: TCGETS: %w", err)
	}
	...
	if err := unix.IoctlSetTermios(fd, unix.TCSETS, tio); err != nil {
		return fmt.Errorf("usbprov: TCSETS: %w", err)
	}
	return nil
}
