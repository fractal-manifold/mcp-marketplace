// Package registry persists per-device configuration for the tokenmonitor-mcp
...
type ConfigPayload struct {
	...
	// config_sync to arm the on-device tmon_ota_* NVS keys (see
	// firmware/components/ota/src/tmon_ota.c). The URL must be HTTPS so
	...
	// signature with one of the pubkeys in tmon_ota_pubkey_{current,next}
	// BEFORE downloading the .bin. See firmware/components/ota/include/
	// tmon_manifest.h for the canonical key order.
	...
	// MinSecureVersion mirrors the device's tmon_min_sv NVS key (packed
...
type Device struct {
	...
	// devices that haven't yet sent the X-Tmon-Serial header (legacy /
	...
	// X-Tmon-Sku header — the eFuse serial's SKU field, or the build-time
	// CONFIG_TMON_SKU for a unit with no factory serial). It is ALWAYS a
...
// device reported via the X-Tmon-Fw-Version header on /sync. Persisting it
...
// not re-stage. Written by tokenmonitor_revert with the version the device is
...
func mergePayload(base, upd ConfigPayload) ConfigPayload {
	...
	// tmon_min_sv (refuses any decrease).
