...
function makeCfg(repoURL, { withKey = true } = {}) {
  ...
  const dir = mkdtempSync(join(tmpdir(), "tmon-otacfg-"));
...
function registryWithDevice(sku, minSV) {
  const dir = mkdtempSync(join(tmpdir(), "tmon-otareg-"));
...
test("check stages update, dry-run previews, idempotent", { skip }, async () => {
  ...
  try {
    ...
    assert.equal(p.firmware_url, "https://dl.example/tmon-S1-0.5.1.bin");
...
test("apiReleasesURL maps github.com to the API and passes through test hosts", () => {
  assert.equal(
    ota.apiReleasesURL("https://github.com/fractal-manifold/tokenmonitor-ota-releases"),
...
test("dev unit stages an immutable dev prerelease via the API", { skip }, async () => {
  ...
  const devIdx = index(canonical, sigB64, { version: DEV_VER, binURL: `https://dl.example/tmon-S1-${DEV_VER}.bin` });
