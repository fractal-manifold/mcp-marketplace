"""MCP stdio server. Implements the 10 tokenmonitor_* tools.
...
def _compat_dir() -> Path:
    ...
    raise RuntimeError("could not locate ../compat/ relative to tmon_mcp module")
...
async def serve(deps: Deps) -> None:
    ...
    server = Server("tokenmonitor-mcp")
...
async def _dispatch(deps: Deps, name: str, args: dict) -> Any:
    if name == "tokenmonitor_status":
        return _status(deps)
    if name == "tokenmonitor_health":
        return await _health(deps)
    if name == "tokenmonitor_recent_logs":
        return _recent_logs(deps, args)
    if name == "tokenmonitor_firmware_logs":
        return await _firmware_logs(deps, args)
    if name == "tokenmonitor_device_logs":
        return _device_logs(deps, args)
    if name == "tokenmonitor_provision_hint":
        return _provision_hint(deps)
    if name == "tokenmonitor_list_devices":
        return _list_devices(deps)
    if name == "tokenmonitor_register_device":
    ...
    if name == "tokenmonitor_discover_devices":
        return await _discover_devices(args)
    if name == "tokenmonitor_provision":
...
def _set_device_pending(deps: Deps, args: dict) -> dict:
    ...
    # built without TMON_OTA_UNSIGNED will refuse an OTA whose pending
...
def _publish_firmware(deps: Deps, args: dict) -> dict:
    ...
    if external:
        ...
        file_name = f"tokenmonitor-{version}.bin"
...
async def _discover_devices(args: dict) -> dict:
    ...
    zc = Zeroconf()
    try:
        ServiceBrowser(zc, "_tmon._tcp.local.", _Listener())
...
async def _provision(deps: Deps, args: dict) -> dict:
    ...
    if not provision_url.endswith("/provision"):
        return {"error": "provision_url must end in /provision (use tokenmonitor_discover_devices to get it)"}
    if len(code) != 6:
