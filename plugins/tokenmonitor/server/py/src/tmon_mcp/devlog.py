...
recent MAX_LINES. The broker (leader) is the sole writer; any tokenmonitor-mcp
process reads the file for the tokenmonitor_device_logs MCP tool. Writes
serialise on a sibling .lock flock and rewrite-then-rename so a reader
always sees a whole file.

Wire-compatible with tokenmonitor-mcp/internal/devlog (Go) and ../js devlog.
"""
...
# Per-device retention cap; matches the tokenmonitor_device_logs limit max.
MAX_LINES = 2000
