...
log = logging.getLogger("tmon_mcp.ota")
...
def pack_semver(v: str) -> int | None:
    ...
    uses for tmon_min_sv (major<<24 | minor<<16 | patch). An optional
...
def valid_version(v: str) -> bool:
    ...
    ValidVersion / JS validVersion and semver_ok() in tmon_manifest.c."""
...
async def _list_dev_releases(session, repo: str) -> list:
    ...
    headers = {
        ...
        "User-Agent": "tokenmonitor-mcp-ota",
    }
...
async def _fetch_index(session, repo: str, sku: str, channel: str = "stable", dev_rels: list | None = None) -> dict:
    ...
    headers = {"Accept": "application/json", "User-Agent": "tokenmonitor-mcp-ota"}
    try:
        async with session.get(url, headers=headers) as resp:
            if resp.status != 200:
...
def _decide(reg: Registry, dev, resolved: dict, dry_run: bool,
    ...
    # refuses only packed(version) STRICTLY BELOW the floor (tmon_ota.c:
    ...
    *,
    ...
    if not o.configured():
        rep["note"] = (
            ...
            "and at least one [[ota.keys]] in tokenmonitor.toml"
        )
        return rep
    if reg is None:
