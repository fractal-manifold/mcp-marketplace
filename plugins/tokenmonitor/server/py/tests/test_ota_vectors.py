...
def _index(canonical: str, sig_b64: str, *, version="0.5.1", bin_url="https://dl.example/tmon-S1-0.5.1.bin") -> dict:
    return {
        "version": version,
...
async def test_check_stages_update(tmp_path):
    ...
    try:
        ...
        assert p.firmware_url == "https://dl.example/tmon-S1-0.5.1.bin"
...
def test_api_releases_url():
    assert ota.api_releases_url("https://github.com/fractal-manifold/tokenmonitor-ota-releases") == \
...
async def test_check_dev_unit_stages_dev_prerelease(tmp_path):
    ...
    dev_idx = _index(canonical, sig_b64, version=dev_ver, bin_url=f"https://dl.example/tmon-S1-{dev_ver}.bin")
...
def _resolved(version: str) -> dict:
    ...
    return {
        ...
        "idx": {
            "bin_url": "https://downloads.example/tmon.bin",
