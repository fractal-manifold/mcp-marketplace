...
func NewServer(d Deps) *server.MCPServer {
	...
	s.AddTool(
		mcp.NewTool("wall_monitor_register_device",
			...
			mcp.WithString("channel",
				mcp.Description("OTA release track for this device: 'stable' (default; follows the newest non-prerelease) or 'dev' (follows the rolling dev tag). Device-level attribute, not pushed to the firmware — it only steers which GitHub asset the broker stages."),
				mcp.Enum("stable", "dev"),
			),
		),
	...
	s.AddTool(
		mcp.NewTool("wall_monitor_set_device_pending",
			...
			mcp.WithNumber("br_night", mcp.Description("Nighttime brightness 5..100.")),
			mcp.WithNumber("vol", mcp.Description("Alert volume 0..100.")),
			mcp.WithString("provider_mode_claude", mcp.Enum("auto", "disabled", "subscription", "api_key"), mcp.Description("Claude provider mode: 'disabled' hides it; 'auto' trusts the broker's credential detection; 'subscription' forces quota-% display; 'api_key' forces $-spend display. Wins over provider_claude.")),
			mcp.WithString("provider_mode_codex", mcp.Enum("auto", "disabled", "subscription", "api_key"), mcp.Description("Codex provider mode (see provider_mode_claude). Wins over provider_codex.")),
			mcp.WithString("provider_mode_gemini", mcp.Enum("auto", "disabled", "subscription", "api_key"), mcp.Description("Gemini provider mode (see provider_mode_claude). Wins over provider_gemini.")),
			mcp.WithBoolean("provider_claude", mcp.Description("Legacy coarse toggle: enable (true→auto) or disable the Claude provider. Prefer provider_mode_claude.")),
			mcp.WithBoolean("provider_codex", mcp.Description("Legacy coarse toggle: enable (true→auto) or disable the Codex provider. Prefer provider_mode_codex.")),
			mcp.WithBoolean("provider_gemini", mcp.Description("Legacy coarse toggle: enable (true→auto) or disable the Gemini provider. Prefer provider_mode_gemini.")),
			...
			mcp.WithString("channel",
				mcp.Description("OTA release track for this device: 'stable' (follows the newest non-prerelease) or 'dev' (follows the rolling dev tag). Applied immediately (device-level attribute, NOT staged in the pending) — it only steers which GitHub asset the broker stages on the next check."),
				mcp.Enum("stable", "dev"),
			),
		),
