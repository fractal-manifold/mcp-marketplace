...
func NewServer(d Deps) *server.MCPServer {
	...
	s.AddTool(
		mcp.NewTool("wall_monitor_set_device_pending",
			...
			mcp.WithString("gemini_models",
				mcp.Description("Comma-separated list of Gemini model IDs to show on the dashboard (max 3). Example: 'gemini-2.5-pro,gemini-2.5-flash'. Empty string clears the override and falls back to the broker's global default (cwm.toml [gemini].models)."),
			),
			...
			mcp.WithString("firmware_manifest_b64",
				mcp.Description("Base64 of the canonical-JSON Ed25519 manifest produced by `tools/cwmtools/lib/manifest.py sign`. The device verifies sig(manifest) BEFORE downloading the .bin and refuses unsigned manifests unless built with CWM_OTA_UNSIGNED=y. Must be supplied together with firmware_manifest_sig_b64."),
			),
	...
	s.AddTool(
		mcp.NewTool("wall_monitor_revert_firmware",
			...
			mcp.WithNumber("target_min_secure_version", mcp.Description("Packed 8.8.16 semver. Broker rejects if below the device's cwm_min_sv mirror.")),
		),
