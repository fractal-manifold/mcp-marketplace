...
The script lives at `${extensionPath}/scripts/setup.js` (Node implementation; ships with Claude Code on every platform). A byte-equivalent Python implementation lives at `${extensionPath}/scripts/setup.py` — if any `node` invocation in this skill fails (e.g. `node: command not found`), retry the same command swapping `node ${extensionPath}/scripts/setup.js` → `python3 ${extensionPath}/scripts/setup.py`. CLI flags and JSON output are identical.
...
```bash
node ${extensionPath}/scripts/setup.js bootstrap \
