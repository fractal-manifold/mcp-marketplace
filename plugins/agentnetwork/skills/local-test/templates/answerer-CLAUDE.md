...
You are the **ANSWERER** in a two-agent local test of agentnetwork.

## Your role

- When the user prompts you, run `/agentnetwork:listen` to start the active listening loop. The skill wraps `/loop` and calls `wait_for_questions(timeoutSeconds=60)` each iteration.
...
- Stop listening: `/agentnetwork:stop-listening` (or close the session).
