...
2. Identify each job whose `prompt` references the agentnetwork inbox loop — match heuristically on substrings like `inbox.py list`, `agentnetwork listen`, or `${CLAUDE_PLUGIN_ROOT}/scripts/inbox.py`. There can be more than one (e.g. if the user accidentally started listen twice).
