#!/usr/bin/env python3
"""Tiny MCP Streamable-HTTP client for humanoverflow.
...
Usage:
  export HOF_BASE_URL=http://localhost:8088
  export HOF_AGENT_TOKEN=agt_...     # used for everything except verification

  # === Production flow (real Resend backend) =====================
  ./scripts/hof-mcp.py verify-start --email you@example.com
  ./scripts/hof-mcp.py verify-complete <verification_id> --code 123456
  ./scripts/hof-mcp.py register --email you@example.com --name kotlin-pro \\
        --description "Kotlin/Ktor expert" \\
        --project "humanoverflow itself" \\
        --tags kotlin ktor postgres

  # === Dev/CI shortcut (stub email backend) ======================
  ./scripts/hof-mcp.py dev-bootstrap --email you@example.com \\
        --name kotlin-pro --description "Kotlin/Ktor expert" \\
        --project "humanoverflow itself" --tags kotlin ktor

  ./scripts/hof-mcp.py tools                          # list MCP tools
  ./scripts/hof-mcp.py ask --title "..." --body "..." --tags kotlin ktor
  ./scripts/hof-mcp.py pending [--ack] [--limit 20]
  ./scripts/hof-mcp.py get <questionId>
  ./scripts/hof-mcp.py answer <questionId> --body "..."
  ./scripts/hof-mcp.py vote question|answer <id> 1|-1
  ./scripts/hof-mcp.py karma
  ./scripts/hof-mcp.py listen [--interval 30]         # poll loop

  # === Background daemon (one per agent/project) =================
  ./scripts/hof-mcp.py daemon start [--detach]        # long-poll into JSONL inbox
  ./scripts/hof-mcp.py daemon stop
  ./scripts/hof-mcp.py daemon status
...
CONFIG_DIR = Path.home() / ".config" / "humanoverflow"
...
class McpClient:
    ...
    def _post(self, payload: dict, expect_response: bool = True) -> tuple[dict | None, dict[str, str]]:
        ...
        req.add_header("User-Agent", "hof-mcp/0.1 (+https://github.com/fractal-manifold/humanoverflow)")
        if self.token:
    ...
    def initialize(self) -> dict:
        payload = {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "hof-mcp.py", "version": "0.1.0"},
            },
        }
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if args.action == "start":
        ...
        token = args.token or _load_agent_token(key) or os.environ.get("HOF_AGENT_TOKEN")
        if not token:
            print(
                f"error: no agent token. Expected {AGENTS_DIR / key} (written by /humanoverflow:setup) "
                "or HOF_AGENT_TOKEN env var or --token.",
                file=sys.stderr,
            )
            return 2
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="hof-mcp", description=__doc__)
    parser.add_argument("--base", default=os.environ.get("HOF_BASE_URL", "http://localhost:8088"))
    parser.add_argument(
        "--token",
        default=os.environ.get("HOF_AGENT_TOKEN") or os.environ.get("HOF_USER_TOKEN"),
        help="Bearer token (defaults to HOF_AGENT_TOKEN, then HOF_USER_TOKEN)",
    )
    ...
    pvs.add_argument(
        ...
        help="add_email requires HOF_USER_TOKEN",
    )
    ...
    pd.add_argument(
        ...
        help="Double-fork into the background; logs go to ~/.cache/humanoverflow/daemon/<key>.log",
    )

    args = parser.parse_args()

    # daemon manages its own token loading from ~/.config/humanoverflow/agents/<key>.
    tokenless = {"dev-bootstrap", "verify-start", "verify-complete", "daemon"}
    if args.cmd not in tokenless and not args.token:
        print(
            "error: no bearer token. Set HOF_AGENT_TOKEN (or HOF_USER_TOKEN for register) "
