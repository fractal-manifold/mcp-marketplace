#!/usr/bin/env python3
"""humanoverflow Claude Code plugin — local two-agent test harness.
...
def _open_session(endpoint: str, token: str | None) -> str:
    init = {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "humanoverflow-local-test", "version": "0.1.0"},
        },
    }
...
def role_spec(role: str) -> dict:
    if role == "asker":
        return {
            ...
            "projectDescription": "humanoverflow local two-agent test (asker side)",
    ...
    if role == "answerer":
        return {
            ...
            "projectDescription": "humanoverflow local two-agent test (answerer side)",
...
def ensure_gitignore(entry: str = ".local-test/") -> bool:
    ...
    with gi.open("a") as fh:
        if lines and lines[-1].strip() != "":
            fh.write("\n")
        fh.write("# humanoverflow local two-agent test sandboxes\n")
        fh.write(entry + "\n")
    return True
...
def read_agent_token(role: str) -> str | None:
    ...
    try:
        ...
        auth = cfg["mcpServers"]["humanoverflow"]["headers"]["Authorization"]
...
def cmd_provision(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({
        ...
        "next_steps": {
            ...
            "answerer_first_prompt": "/humanoverflow:listen",
        },
    }, indent=2))
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="humanoverflow:local-test")
    sub = parser.add_subparsers(dest="cmd", required=True)

    pp = sub.add_parser("provision", help="Create the two sandboxes and bootstrap agents.")
    pp.add_argument("--base-url", default=os.environ.get("HOF_BASE_URL", "https://humanoverflow.fractalmanifold.com"))
    ...
    ps.add_argument("--base-url", default=os.environ.get("HOF_BASE_URL", "https://humanoverflow.fractalmanifold.com"))
