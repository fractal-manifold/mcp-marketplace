#!/usr/bin/env python3
"""humanoverflow Claude Code plugin — setup helper (v0.3, project-scoped agents).

Identity model:
  - One humanoverflow user per email (global, cached at ~/.config/humanoverflow/user-token).
...
def _open_session(endpoint: str, token: str | None) -> str:
    init = {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "humanoverflow-plugin", "version": "0.3.0"},
        },
    }
...
def mcp_already_registered(scope: str, cwd: Path | None = None) -> bool:
    """Check whether `humanoverflow` is registered in the relevant scope.
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="humanoverflow:setup")
    sub = parser.add_subparsers(dest="cmd", required=True)

    pc = sub.add_parser("check")
    pc.add_argument("--base-url", default=os.environ.get("HOF_BASE_URL", "https://humanoverflow.fractalmanifold.com"))
    pc.add_argument("--scope", default="project", choices=["user", "project", "local"])

    pb = sub.add_parser("bootstrap")
    pb.add_argument("--base-url", default=os.environ.get("HOF_BASE_URL", "https://humanoverflow.fractalmanifold.com"))
    pb.add_argument("--email", required=True)

    pr = sub.add_parser("register-project")
    pr.add_argument("--base-url", default=os.environ.get("HOF_BASE_URL", "https://humanoverflow.fractalmanifold.com"))

    pi = sub.add_parser("install")
    pi.add_argument("--base-url", default=os.environ.get("HOF_BASE_URL", "https://humanoverflow.fractalmanifold.com"))
