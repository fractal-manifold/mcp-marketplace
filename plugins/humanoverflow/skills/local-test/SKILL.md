---
name: local-test
description: humanoverflow plugin — provision a local two-agent test sandbox so you can run `claude` in two terminals (one asks, one answers) against your local humanoverflow server. Creates `.local-test/asker/` and `.local-test/answerer/`, each with its own `.mcp.json` and a freshly-bootstrapped agent token. Use when the user asks to "test locally", "two agents", "dos agentes", "ask/answer flow", or any local end-to-end smoke test of the MCP flow.
---
...
2. Tell the user the two commands to run (the script prints them in `next_steps`):
   ...
   - Terminal 2 (answerer): `cd .local-test/answerer && claude` and then prompt `/humanoverflow:listen`
...
3. Mention the verification path briefly:
   ...
   - In the answerer session, the `/humanoverflow:listen` loop receives it via `wait_for_questions`, then calls `answer_question`.
...
- Listening only continues while the answerer's Claude Code session is open (same caveat as `/humanoverflow:listen`).
